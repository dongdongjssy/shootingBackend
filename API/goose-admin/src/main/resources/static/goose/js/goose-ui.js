...
(function ($) {
    $.extend({
    	...
    	form: {
            ...
            updatePlaceSelects: function(countyId, cityId, provId) {
            	if($.common.isNotEmpty(provId)) {
            		$.ajax({
			            url: "/place/province/"+$(provId).val()+"/list",		            
			            type: "get",
			            processData: false,
			            contentType: false,
			            success: function(result) {
			            	$(cityId).html("");
			                $.each(result.cityList, function(i, v) {
			            		$(cityId).append("<option value=\""+v.id+"\">"+v.name+"</option>");
			                })
			                
			            	$(countyId).html("");
			                $.each(result.countyList, function(i, v) {
			            		$(countyId).append("<option value=\""+v.id+"\">"+v.name+"</option>");
			                })
			            }
            		})
            	}
            	else if($.common.isNotEmpty(cityId)) {
    		    	$.ajax({
    		            url: "/place/city/"+$(cityId).val()+"/list",		            
    		            type: "get",
    		            processData: false,
    		            contentType: false,
    		            success: function(result) {
        ...
        operate: {
            ...
            exportSelected: function() {
        		...
        		var data = { "ids": rows.join() };
        		  ...
        		  //data 是id逗号隔开，如34,33,21  后台可以用in语句 in (34,33,21)
        		//$.operate.submit(url, "post", "json", data);
        		$.modal.loading("正在导出数据，请稍后...");
        		
        		var ids = rows?data:{}
        		//
    			$.post($.table._option.exportUrl,ids , function(result) {
    				if (result.code == web_status.SUCCESS) {
    			        window.location.href = ctx + "common/download?fileName=" + result.msg + "&delete=" + true;
    				} else {
    					$.modal.alertError(result.msg);
            ...
            // 成功结果提示msg（父窗体全局更新）
            ajaxSuccessClose: function (result) {
            	if (result.code == web_status.SUCCESS) {
